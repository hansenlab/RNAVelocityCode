...
panel_a.p <- plot_grid(mums.p, spliced_t.p, unspliced_t.p,
											 ms_t.p, mu_t.p,
											 
										 dynamical_t.p,
										 dynamical_v.p,
													 nrow = 1, ncol = 7, label_size = 10, labels = c("a"))
...
gene_vec_length.v <- sqrt(colSums(assay(dynamical.o, "velocity") ^ 2))
...
range.v <- c(min(tmp.df[, c("x", "y", "z")]), max(tmp.df[, c("x", "y", "z")]))
...
length_com.p <- ggplot(data = tmp.df, aes(x = x, y = y, color = color)) +
	...
	labs(x = "Vector length by projection", y = "Vector length by\ntransition probability", title = "Unscaled vector length per cell") +
	theme(legend.position = c(0, 1),
				legend.justification = c(0, 1),
				legend.title = element_blank())

length_com2.p <- ggplot(data = tmp.df, aes(x = z, y = x, color = color)) +
	geom_abline(slope = 1, intercept = 0, size = 0.6, linetype = "dashed", color = "grey") +
	geom_scattermore(pointsize = 5.01,  alpha = 0.8) +
	scale_color_manual(values = c("black", "red"), name = "",
										 labels = c(str_c("Other cell (n=", ncol(sce.o) - length(cell.idx), ")"), str_c("Cells in the box (n=",length(cell.idx), ")"))) + 
	labs(x = "High dimensional velocity length", y = "Vector length by\nprojection", title = " ") +
	theme(legend.position = "none",
				legend.justification = c(0, 1),
				legend.title = element_blank()) +
	xlim(range.v) + ylim(range.v) +
	annotate(geom = "text", x = 0, y = 3.5, size = 2, hjust = 0, vjust = 1,
					 label = str_c("PCC: ", sprintf("%.3f", cor(tmp.df$z, tmp.df$x, use = "complete"))), parse = FALSE) +
	annotate(geom = "text", x = 0, y = 3, size = 2, hjust = 0, vjust = 1,
					 label = str_c("RMSE: ", sprintf("%.3f", Metrics::rmse(tmp.df$z, tmp.df$x))), parse = FALSE)

length_com3.p <- ggplot(data = tmp.df, aes(x = z, y = y, color = color)) +
	geom_abline(slope = 1, intercept = 0, size = 0.6, linetype = "dashed", color = "grey") +
	geom_scattermore(pointsize = 5.01,  alpha = 0.8) +
	scale_color_manual(values = c("black", "red"), name = "",
										 labels = c(str_c("Other cell (n=", ncol(sce.o) - length(cell.idx), ")"), str_c("Cells in the box (n=",length(cell.idx), ")"))) + 
	...
	xlim(range.v) + ylim(range.v) +
	annotate(geom = "text", x = 0, y = 3.5, size = 2, hjust = 0, vjust = 1,
					 label = str_c("PCC: ", sprintf("%.3f", cor(tmp.df$z, tmp.df$y, use = "complete"))), parse = FALSE) +
	annotate(geom = "text", x = 0, y = 3, size = 2, hjust = 0, vjust = 1,
					 label = str_c("RMSE: ", sprintf("%.3f", Metrics::rmse(tmp.df$z, tmp.df$y))), parse = FALSE)
...
panel_bc.p <- plot_grid(true_vec_umap.p, true_proj_umap.p, small_umap.p, 
												nrow = 1, ncol = 3, label_size = 10, labels = c("b", "", "c"))
...
mp <- plot_grid(panel_a.p, panel_bc.p, panel_defgh.p, 
								rel_heights = c(0.5, 1, 2),
										 nrow = 3, ncol = 1, label_size = 10, labels = NULL)

save_plot(here::here("figs", "main", "main.predict.pdf"), mp,
					base_height = 2 *1.2 / 1.4, base_width = 2*1.4 / 1.4, nrow = 3.5, ncol = 3, device = cairo_pdf)
