...
nan.idx <- which(!(rownames(sce.o) %in% metadata(sce.o)$top.hvgs)) #   which(!rowData(sce.o)$velocity_genes) # 
...
p2 <- plotEmbScat(new.o, "UMAP", title = "Only using Ms", x_lab = "UMAP-1", y_lab = "UMAP-2", color_by = "clusters", labels.v = levels(sce.o$clusters),
									color.name = "Cell_type", colors.v = metadata(sce.o)$clusters_colors) + 
...
save_plot(here::here("figs", "pancreas", str_c("only_using_Ms.pdf")), mp,
