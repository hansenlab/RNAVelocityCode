...
make_sim <- function(noise_level = 1,
										 n_obs = 500L,
										 n_vars = 10L) {
...
sce.o <-
	make_sim(noise_level = noise_level,
					 n_obs = 500L,
					 n_vars = new_nvar.v)
...
true_t.p <-
	plotTLoess(
		x = tmp.df$x,
		y = tmp.df$ms,
		x_lab = "True latent time",
		x.pos = 0.55,
		y.pos = 0.9,
		point.size = 5.01,
		color_var = tmp.df$x,
		y_lab = TeX(r'(Ms$_g$)'),
		title = str_c("Noise level ", noise_level),
		scale_color = scale_color_gradientn(
			name = "t",
			limits = range(tmp.df$x),
			colors = c(
				"#440154",
				"#482576",
				"#414487",
...
tmp.df$name <-
	factor(tmp.df$name,
				 levels = (
				 	tmp.df %>% filter(su == "u") %>% group_by(name) %>%
				 		summarize(medians = median(r2, na.rm = TRUE)) %>% arrange(desc(medians))
				 )$name)
...
save_plot(
	here::here("figs", "main", "main.r2.pdf"),
	panel_abcde.p,
