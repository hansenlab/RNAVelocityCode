...
grid.df <-
	gridVectors(reducedDim(sce.o, "UMAP"), embedded, resolution = 30)
...
pancreas_full.p <-
	...
	scale_color_manual(
		values = metadata(sce.o)$clusters_colors,
		name = "Cell_type",
		labels = levels(sce.o$clusters),
...
grid_subset.df <-
	gridVectors(reducedDim(new.o, "UMAP"), embedded_subset, resolution = 30)
colnames(grid_subset.df) <-
	c("start.1", "start.2", "end.1", "end.2")
...
pancreas_legend.p <-
	get_legend(
		plotEmbScat(
			...
			y_lab = "UMAP-2",
			color_by = "clusters",
			labels.v = levels(sce.o$clusters),
			color.name = "Cell_type",
			colors.v = metadata(sce.o)$clusters_colors
		) +
...
### compare direction
cells.v <-
	colnames(sce.o)[which((sce.o$clusters == "Pre-endocrine") &
...
data.df <-
	rbind(
		data.frame(theta = theta.v[cells.v], type = "full"),
...
dir_polar.p <- ggplot(tmp.df, aes(x = x, y = y * 0.5 + 2)) +
	...
	labs(
		title = str_c("Density of directions\n(n=", length(cells.v), ")"),
...
save_plot(
	here::here("figs", "main", "main.pancreas.pdf"),
	mp,
