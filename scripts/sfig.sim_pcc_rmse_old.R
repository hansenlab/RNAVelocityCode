...
tmp.df$name <- factor(tmp.df$name, levels = c("ms", "steady", "dynamical"))
...
pcc.p <- ggplot(tmp.df, aes(x = noise, y = pcc, color = name,  dodge = name, fill = type)) +
	...
	scale_color_manual(values = c("#FF7F00", "#377EB8", "#4DAF4A"), name = "Type", limits = c("ms", "steady", "dynamical"), labels = c("Ms", "Steady v", "Dynamical V")) +
...
pcc.df <- do.call(rbind, lapply(tmp2.l, "[[", "pcc"))
tmp.df <- pcc.df %>% pivot_longer(
	cols = 1:3,
	values_to = "pcc"
)
...
rmse.df <- do.call(rbind, lapply(tmp2.l, "[[", "rmse"))
tmp.df <- rmse.df %>% pivot_longer(
	cols = 1:3,
	values_to = "rmse"
)
