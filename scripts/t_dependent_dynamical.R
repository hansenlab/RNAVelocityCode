...
phase.df <- dynamical_phase(alpha = rowData.df$fit_alpha[6], beta = rowData.df$fit_beta[6],
														gamma = rowData.df$fit_gamma[6], scaling = rowData.df$fit_scaling[6], t_ = rowData.df$fit_t_[6],
														u0 = rowData.df$fit_u0[6], s0 = rowData.df$fit_s0[6], t = assay(sce.o, "fit_t")[6, ])
ymax.v <- rowData.df$fit_gamma[6] / rowData.df$fit_beta[6] * (max(phase.df$st) - min(phase.df$st)) + min(phase.df$ut)
...
rowData2.df <- rowData(sce2.o)
phase2.df <- dynamical_phase(alpha = rowData2.df$fit_alpha[6], beta = rowData2.df$fit_beta[6],
														gamma = rowData2.df$fit_gamma[6], scaling = rowData2.df$fit_scaling[6], t_ = rowData2.df$fit_t_[6],
														u0 = rowData2.df$fit_u0[6], s0 = rowData2.df$fit_s0[6], t = assay(sce2.o, "fit_t")[6, ])
ymax2.v <- rowData2.df$fit_gamma[6] / rowData2.df$fit_beta[6] * (max(phase2.df$st) - min(phase2.df$st)) + min(phase2.df$ut)

p5 <- plotxy(assay(sce2.o, "spliced")[6, ], assay(sce2.o, "unspliced")[6, ], title = "Time-dependent \u03B3", x_lab = "Spliced",
						 y_lab = "Unspliced", color_by = "true_t", point.size = 5.01,
						 colors.v = sce.o$true_t,
						 hue.colors = c("#440154", "#482576", "#414487","#35608D",
						 							 "#2A788E", "#21908C","#22A884", "#43BF71",
						 							 "#7AD151", "#BBDF27", "#FDE725"), color.name = "t") +
	geom_path(data = phase2.df, aes(x = st, y = ut), color = "#800080") +
	geom_segment(aes(x = min(phase2.df$st), xend = max(phase2.df$st), y = min(phase2.df$ut), yend = ymax2.v), linetype = "dashed", color = '#800080') +
...
p6 <- plotxy2(x = de2.v, assay(sce2.o, "velocity")[, 50], title = "Time-dependent \u03B3", x_lab = "Difference of Ms between two cells",
...
p7 <- plotVelocityStream(sce3.o, embedded3[, 1:2], color_by = "true_t", color.alpha = 0.7, grid.resolution = 15,
	...
	labs( y = "PCA-2", x = "PCA-1") +
	ggtitle("Different \u03B1 and time-dep. \u03B3") +
	.theme_noframe + theme(legend.position = "none",
												 legend.justification = c(1, 1))

rowData3.df <- rowData(sce3.o)
phase3.df <- dynamical_phase(alpha = rowData3.df$fit_alpha[6], beta = rowData3.df$fit_beta[6],
														 gamma = rowData3.df$fit_gamma[6], scaling = rowData3.df$fit_scaling[6], t_ = rowData3.df$fit_t_[6],
														 u0 = rowData3.df$fit_u0[6], s0 = rowData3.df$fit_s0[6], t = assay(sce3.o, "fit_t")[6, ])
ymax3.v <- rowData3.df$fit_gamma[6] / rowData3.df$fit_beta[6] * (max(phase3.df$st) - min(phase3.df$st)) + min(phase3.df$ut)
...
p8 <- plotxy(assay(sce3.o, "spliced")[6, ], assay(sce3.o, "unspliced")[6, ], title = "\u03B1 = 30, time-dep. \u03B3", x_lab = "Spliced",
						 ...
						 hue.colors = c("#440154", "#482576", "#414487","#35608D",
						 							 "#2A788E", "#21908C","#22A884", "#43BF71",
						 							 "#7AD151", "#BBDF27", "#FDE725"), color.name = "t") +
	geom_path(data = phase3.df, aes(x = st, y = ut), color = "#800080") +
	geom_segment(aes(x = min(phase3.df$st), xend = max(phase3.df$st), y = min(phase3.df$ut), yend = ymax3.v), linetype = "dashed", color = '#800080') +
...
p9 <- plotxy2(x = de3.v, assay(sce3.o, "velocity")[, 50], title = "\u03B1 = 5 or 30, time-dep. \u03B3", x_lab = "Difference of Ms between two cells",
							y_lab = "Estimated velocity", point.size = 8.01, color_var = factor(c(1, 1, 0, 0, 0, 1, rep(0, 4))),
...
save_plot(here::here("figs", "t_dependent", "t_dependent_dynamical.pdf"), mp,
