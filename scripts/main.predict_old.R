...
steady_vec.p <- plotVelocityStream(steady.o, embedded[, 1:2], use.dimred = "X_pca", color_by = "true_t", color.alpha = 0.7, grid.resolution = 8,
																	 ...
																	 arrow.angle = 25, arrow.length = 0.2, point.size = 4.01) +
...
panel_d.p <- plotEmbExp(sce.o, "X_pca", color.v = sce.o$true_t,
												title = "True v. cos.sim", x_lab = "PCA-1", 
												y_lab = "PCA-2", color.name = "time", point.size = 4.01,
												point.alpha = 0.6) + 
	geom_segment(data=grid.df, mapping=aes(x=start.1, y=start.2, xend=end.1, yend=end.2), arrow = arrow(length=unit(0.02, "inches")), inherit.aes = FALSE, size = 0.27)  +
...
panel_e.p <- plotEmbExp(sce.o, "X_pca", color.v = sce.o$true_t,
...
panel_h.p <- plotEmbExp(sce.o, "UMAP", color.v = sce.o$true_t, title = "True v. cos.sim", x_lab = "UMAP-1", y_lab = "UMAP-2", color.name = "time", point.size = 4.01) + 
...
panel_i.p <- plotEmbExp(sce.o, "UMAP", color.v = sce.o$true_t, title = "True v. projection", x_lab = "UMAP-1", y_lab = "UMAP-2", color.name = "t", point.size = 4.01) + 
	geom_segment(data=grid.df, mapping=aes(x=start.1, y=start.2, xend=end.1, yend=end.2), arrow = arrow(length=unit(0.03, "inches")), inherit.aes = FALSE, size = 0.22) +
	annotate("rect", xmin= -3, xmax= 0.3, ymin= - 10.5 , ymax= - 8, alpha = 0.2, color="red", fill= NA, linetype = "dashed", size = 0.3) + 
	.theme_noframe +
	theme(legend.position = "none")
