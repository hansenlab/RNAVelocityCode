...
ms_t.p <- plotxy(tmp.df$x, tmp.df$ms, title = "Using learned k-NN", x_lab = "True t.",
...
mu_t.p <- plotxy(tmp.df$x, tmp.df$mu, title = "Using learned k-NN", x_lab = "True t.",
...
spliced_t.p <- plotxy(tmp.df$x, tmp.df$spliced, title = "Using learned k-NN", x_lab = "True t.",
...
unspliced_t.p <- plotxy(tmp.df$x, tmp.df$unspliced, title = "Using learned k-NN", x_lab = "True t.",
...
dynamical_t.p <- plotxy(tmp.df$x, tmp.df$dynamical_v, title = "Using learned k-NN", x_lab = "True t.",
...
dynamical_v.p <- plotxy(tmp.df$v, tmp.df$dynamical_v, title = "Using learned k-NN", x_lab = "True v.",
												y_lab = "Dynamical v.", color_by = "true_t", point.size = 4.01,
												colors.v = tmp.df$x,
												hue.colors = c("#440154", "#482576", "#414487","#35608D",
																			 "#2A788E", "#21908C","#22A884", "#43BF71",
																			 "#7AD151", "#BBDF27", "#FDE725"), color.name = "t") +
	theme(legend.position = "none") +
	annotate(geom = "text", x = .percent_range(tmp.df$v, 0.6),
...
est_t.p <- plotxy(tmp.df$x, tmp.df$est, title = "Using learned k-NN", x_lab = "True t.",
...
ms_t.p <- plotxy(tmp.df$x, tmp.df$ms, title = "Using true k-NN", x_lab = "True t.",
								 y_lab = "Ms", color_by = "true_t", point.size = 4.01,
								 colors.v = tmp.df$x,
								 hue.colors = c("#440154", "#482576", "#414487","#35608D",
								 							 "#2A788E", "#21908C","#22A884", "#43BF71",
...
mu_t.p <- plotxy(tmp.df$x, tmp.df$mu, title = "Using true k-NN", x_lab = "True t.",
								 y_lab = "Mu", color_by = "true_t", point.size = 4.01,
								 colors.v = tmp.df$x,
								 hue.colors = c("#440154", "#482576", "#414487","#35608D",
								 							 "#2A788E", "#21908C","#22A884", "#43BF71",
...
spliced_t.p <- plotxy(tmp.df$x, tmp.df$spliced, title = "Using true k-NN", x_lab = "True t.",
											y_lab = "Spliced", color_by = "true_t", point.size = 4.01,
											colors.v = tmp.df$x,
											hue.colors = c("#440154", "#482576", "#414487","#35608D",
																		 "#2A788E", "#21908C","#22A884", "#43BF71",
...
unspliced_t.p <- plotxy(tmp.df$x, tmp.df$unspliced, title = "Using true k-NN", x_lab = "True t.",
												y_lab = "Unspliced", color_by = "true_t", point.size = 4.01,
												colors.v = tmp.df$x,
												hue.colors = c("#440154", "#482576", "#414487","#35608D",
																			 "#2A788E", "#21908C","#22A884", "#43BF71",
...
dynamical_t.p <- plotxy(tmp.df$x, tmp.df$dynamical_v, title = "Using true k-NN", x_lab = "True t.",
												y_lab = "Dynamical v.", color_by = "true_t", point.size = 4.01,
												colors.v = tmp.df$x,
												hue.colors = c("#440154", "#482576", "#414487","#35608D",
																			 "#2A788E", "#21908C","#22A884", "#43BF71",
...
dynamical_v.p <- plotxy(tmp.df$v, tmp.df$dynamical_v, title = "Using true k-NN", x_lab = "True v.",
												y_lab = "Dynamical v.", color_by = "true_t", point.size = 4.01,
												colors.v = tmp.df$x,
												hue.colors = c("#440154", "#482576", "#414487","#35608D",
																			 "#2A788E", "#21908C","#22A884", "#43BF71",
...
est_t.p <- plotxy(tmp.df$x, tmp.df$est, title = "Using true k-NN", x_lab = "True t.",
									y_lab = "Estimated t.", color_by = "true_t", point.size = 5.01,
									colors.v = tmp.df$x,
									hue.colors = c("#440154", "#482576", "#414487","#35608D",
																 "#2A788E", "#21908C","#22A884", "#43BF71",
...
steady_len_est.v <- sqrt(colSums(assay(steady.o, "velocity")[row.idx, ] ^ 2))
len_true.v <- sqrt(colSums(true_v.m[row.idx, ] ^ 2))

row.idx <- which(!rowAnyNAs(assay(dynamical.o, "velocity")))
dynamical_len_est.v <- sqrt(colSums(assay(dynamical.o, "velocity")[row.idx, ] ^ 2))
...
row.idx <- which(!rowAnyNAs(assay(steady.o, "velocity")))
steady_len_est_trueknn.v <- sqrt(colSums(assay(steady.o, "velocity")[row.idx, ] ^ 2))

row.idx <- which(!rowAnyNAs(assay(dynamical.o, "velocity")))
dynamical_len_est_trueknn.v <- sqrt(colSums(assay(dynamical.o, "velocity")[row.idx, ] ^ 2))

### 4 panels showing example
steady_leanred.p <- plotxy(len_true.v, steady_len_est.v, title = "Steady v. length using learned k-NN", x_lab = "Vector length of true v.",
													 y_lab = "Vector length of steady v.", color_by = "true_t", point.size = 5.01,
	...
	theme(legend.position = c(1, 0),
				legend.justification = c(1, 0)) +
	annotate(geom = "text", x = .percent_range(len_true.v, 0.05),
					 y = .percent_range(steady_len_est.v, 0.95), size = 2, hjust = 0, vjust = 1,
					 label = str_c("PCC: ", sprintf("%.3f", cor(len_true.v, steady_len_est.v))), parse = FALSE) 

steady_true.p <- plotxy(len_true.v, steady_len_est_trueknn.v, title = "Steady v. length using true k-NN", x_lab = "Vector length of true v.",
												y_lab = "Vector length of steady v.", color_by = "true_t", point.size = 5.01,
												colors.v = steady.o$true_t,
												hue.colors = c("#440154", "#482576", "#414487","#35608D",
																			 "#2A788E", "#21908C","#22A884", "#43BF71",
	...
	annotate(geom = "text", x = .percent_range(len_true.v, 0.05),
					 y = .percent_range(steady_len_est.v, 0.95), size = 2, hjust = 0, vjust = 1,
					 label = str_c("PCC: ", sprintf("%.3f", cor(len_true.v, steady_len_est_trueknn.v))), parse = FALSE) 



dynamical_leanred.p <- plotxy(len_true.v, dynamical_len_est.v, title = "Dynamical v. length using learned k-NN", x_lab = "Vector length of true v.",
															y_lab = "Vector length of dynamical v.", color_by = "true_t", point.size = 5.01,
	...
	theme(legend.position = "none") +
	annotate(geom = "text", x = .percent_range(len_true.v, 0.05),
					 y = .percent_range(dynamical_len_est.v, 0.95), size = 2, hjust = 0, vjust = 1,
					 label = str_c("PCC: ", sprintf("%.3f", cor(len_true.v, dynamical_len_est.v))), parse = FALSE) 

dynamical_true.p <- plotxy(len_true.v, dynamical_len_est_trueknn.v, title = "Dynamical v. length using true k-NN", x_lab = "Vector length of true v.",
													 y_lab = "Vector length of dynamical v.", color_by = "true_t", point.size = 5.01,
													 colors.v = dynamical.o$true_t,
													 hue.colors = c("#440154", "#482576", "#414487","#35608D",
													 							 "#2A788E", "#21908C","#22A884", "#43BF71",
	...
	annotate(geom = "text", x = .percent_range(len_true.v, 0.05),
					 y = .percent_range(dynamical_len_est.v, 0.95), size = 2, hjust = 0, vjust = 1,
					 label = str_c("PCC: ", sprintf("%.3f", cor(len_true.v, dynamical_len_est_trueknn.v))), parse = FALSE) 


row3.p <- plot_grid(dynamical_leanred.p + ggtitle("Using learned k-NN") + ylab("Vector length of ets. v"),
										dynamical_true.p + ggtitle("Using true k-NN") + ylab("Vector length of ets. v"),
										nrow = 1, ncol = 3, label_size = 10, labels = c("g", "h"))





mp <- plot_grid(panel_efg.p, panel_mno.p, row3.p,
								nrow = 3, ncol = 1, label_size = 10, labels = NULL)

save_plot(here::here("figs", "main", "main.noise_genes.pdf"), mp,
					base_height = 2 *1.2 / 1.4, base_width = 2*1.4 / 1.4, nrow = 3 * 0.9, ncol = 3, device = cairo_pdf)
...
mp <- plot_grid(steady_leanred.p, steady_true.p,
								dynamical_leanred.p, dynamical_true.p,
								
										nrow = 2, ncol = 2, label_size = 10, labels = c("auto"))

save_plot(here::here("figs", "sfigs", "sfig.length_com.pdf"), mp,
					base_height = 2 *1.2 / 1.2, base_width = 2*1.4 / 1.2, nrow = 2, ncol = 2, device = cairo_pdf)
