...
umap.o <- qread(here::here("data/pbmc68k/umap.o.qs"))
...
sce.o$clusters <- factor(sce.o$celltype)
metadata(sce.o)$clusters_colors <- brewer.pal(nlevels(factor(sce.o$celltype)), "Set3")
...
grid.df <- gridVectors(reducedDim(sce.o, "UMAP"), embedded)
colnames(grid.df) <- c("start.1", "start.2", "end.1", "end.2")
p2 <- plotEmbScat(sce.o, "UMAP", title = "Vector fields (umap.predict)", x_lab = "UMAP-1", y_lab = "UMAP-2", color_by = "clusters", labels.v = levels(sce.o$clusters),
									color.name = "Cell_type", colors.v = metadata(sce.o)$clusters_colors) + 
	geom_segment(data=grid.df, mapping=aes(x=start.1, y=start.2, xend=end.1, yend=end.2), arrow = arrow(length=unit(0.02, "inches")), inherit.aes = FALSE, size = 0.2)
...
colnames(grid.df) <- c("start.1", "start.2", "end.1", "end.2")
p3 <- plotEmbScat(sce.o, "new.UMAP", title = "Vector fields on subset data ", x_lab = "UMAP-1", y_lab = "UMAP-2", color_by = "clusters", labels.v = levels(sce.o$clusters),
									color.name = "Cell_type", colors.v = metadata(sce.o)$clusters_colors) + 
	geom_segment(data=grid.df, mapping=aes(x=start.1, y=start.2, xend=end.1, yend=end.2), arrow = arrow(length=unit(0.02, "inches")), inherit.aes = FALSE, size = 0.2)
...
grid.df <- gridVectors(reducedDim(sce.o, "new.UMAP"), embedded)
colnames(grid.df) <- c("start.1", "start.2", "end.1", "end.2")
p4 <- plotEmbScat(sce.o, "new.UMAP", title = "Vector fields (umap.predict on subset data)", x_lab = "UMAP-1", y_lab = "UMAP-2", color_by = "clusters", labels.v = levels(sce.o$clusters),
									color.name = "Cell_type", colors.v = metadata(sce.o)$clusters_colors) + 
	geom_segment(data=grid.df, mapping=aes(x=start.1, y=start.2, xend=end.1, yend=end.2), arrow = arrow(length=unit(0.02, "inches")), inherit.aes = FALSE, size = 0.2)
...
save_plot(here::here("figs", "pbmc68k", str_c("umap_predict.pdf")), mp,
